...
var circleDonutChart = function(chartElementID) {
  ...
  function textSetter(DOM, text) {
    ...
    if (text >= 1000 && (textScaling = 1)) {
      textScaling =
        1 - Math.floor(Math.log(text) / Math.log(10) - 2) / 10 - 0.1;
  ...
  function onLoad() {
    ...
    if (chart == null) {
      ...
      if (waitingLoad > 20000) {
        throw 'circleDonutChart: onLoad() - The chart element "' +
          chartID +
          '" could not be loaded in the last 20 seconds';
      }
    ...
    innerCircleDOM = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'circle'
    );
    outerCircleDOM = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'path'
    );
    textDOM = document.createElementNS('http://www.w3.org/2000/svg', 'text');
    tNumberDOM = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'tspan'
    );
  ...
  this.draw = function(options) {
    ...
    if (options.end > maxValue) options.end = maxValue;
    ...
    switch (titlePosition) {
      case 'inner-bottom':
        if (!oTester.titleColor) titleColor = '#ffffff';
        titleDOM.setAttribute('y', centery + sizey / 5 + textShift / 2);
        break;

      case 'outer-bottom':
        if (!oTester.titleColor) titleColor = '#000';
      ...
      default:
      case 'outer-top':
        if (!oTester.titleColor) titleColor = '#666666';
    ...
    if (animationSpeed > 0) {
      ...
      tNumberDOM.textContent =
        Math.round((startValue / 360) * maxValue * 10) / 10;
      checkVisible();
    } else {
      ...
      tNumberDOM.textContent =
        Math.round((options.end / 360) * maxValue * 10) / 10;
    }
    lastValue = endValue;
  };
};
...
circle.draw({
  ...
  start: 0,
  maxValue: 100,
  titlePosition: 'outer-bottom',
  titleText: '115 of 115 Goals Completed',
